...
class GraphLoader {
	...
	function GraphLoader() {
		...
		ExternalInterface.addCallback("getProperty", this, fetchProperty);
		
		
		ExternalInterface.addCallback("addNode", this, jsAddNode);
		ExternalInterface.addCallback("addJustNode", this, jsAddJustNode);
		ExternalInterface.addCallback("addRandomNode", this, addRandomNode);
		ExternalInterface.addCallback("addEdge", this, jsAddEdge);
	...
	public function BuildGraph(success) {
		...
		if(GraphSegment != null && GraphSegment != undefined) {_root.Graph.idealSegmentLength = Number(GraphSegment);}
		...
		_root.CSBrand.onPress = function() {getURL("http://www.creativesynthesis.net");};
	...
	public function pressHandler(targetMC:MovieClip):Void {
		...
		ExternalInterface.call("nodeNotify", returnString.substr(4,3), targetMC._parent.content.text); 
	...
	function jsAddNode(jId,jNodeType,jNodeCluster,jNodeText,jLinkValue,jImageValue,jScaleValue,jTextColor,jBackgroundColor,jEdgeId,jTarget,jLabel,jLabelColor) {
		...
		//Node Stuff
		jsAddJustNode(jId,jNodeType,jNodeCluster,jNodeText,jLinkValue,jImageValue,jScaleValue,jTextColor,jBackgroundColor);
			
		//Edge Stuff
		jsAddEdge(jId,jTarget,jLabel,jLabelColor);
	...
	function jsAddJustNode(jId,jNodeType,jNodeCluster,jNodeText,jLinkValue,jImageValue,jScaleValue,jTextColor,jBackgroundColor) {
		...
		// Error check and set defaults
		if(nodeCluster == undefined || nodeCluster == "" || nodeCluster == "null") { nodeCluster = "default";}			
		if(nodeType == undefined || nodeType == "" || nodeType == "null") { nodeType = "CircleNode";}
		if(nodeText == undefined || nodeText == "" || nodeText == "null") { nodeText = null; }
		if(linkValue == undefined || linkValue == "" || linkValue == "null") { linkValue = null; }
		if(imageValue == undefined || imageValue == "" || imageValue == "null") { imageValue = null; }
		if(scaleValue == undefined || scaleValue == "" || scaleValue == "null") {scaleValue = 100;}
		
		var bFirst = nList.length == 0 ? true : false;
	...
	function jsAddEdge(jId,jTarget,jLabel,jLabelColor) {
		
		
		
		//Edge Stuff
		var id:String = jId;	
		var target:String = jTarget;
		var label:String = jLabel;
		var labelColor:String = jLabelColor;
		...
		// Setup Edge
		addEdge(id, target, label, jLabelColor);
